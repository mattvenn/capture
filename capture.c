...
int main (int argc, char **argv) {
  ...
  while (-1 != (ch = getopt(argc, argv, "n:o:"))) {
    switch (ch) {
    case 'n':
      max_samp = strtod(optarg, NULL);
      break;
    case 'o':
      fname = optarg;
      break;
  ...
  if (0 != prussdrv_open(PRU_EVTOUT_0)) {
    fprintf(stderr, "prussdrv_open() failed. (Did you forget to run setup.sh?)\n");
    return EXIT_FAILURE;
  }
